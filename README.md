...
> **构建人机协同的高阶数字化生产力骨架。** 本仓库集成 9 个核心专业级 AI Agent 角色（含 1 个量化业务 Lead 调度 15 子角色）与 45+ 个高度适配、开箱即用的配套 Skill 配置文件，形成工业级的全生命周期软件开发与量化投研设计规范。
...
graph TB
    ...
    QT --> Q0["📈 量化研究员<br/>15 角色调度者"]:::member
    QT --> L1["分析师层 (7)<br/>市场/新闻/社媒/基本面/A股/舆情/全球宏观"]:::member
...
> **量化团队 15 角色详情**（反向工程自 [QuantAgents/tradingagents](https://github.com/cober2018/agent-promptskills) 同源项目）：[完整列表见 `./agents/quant/quant-researcher.md` 的 5 层图谱](./agents/quant-researcher.md#5-层派工图谱)。
...
| **📈 量化团队** | 量化投研：5 层 15 角色图谱（TradingAgents 风格） | 量化研究员（Lead + 调度者）、<span style="color:#4f46e5">**15 角色**</span>：7 分析师 + 2 研究员 + 2 经理 + 1 交易员 + 3 风控辩手、<span style="color:#be185d">数据工程师（共享）</span> |
...
| 层级 | 数量 | 角色 | 提示词路径 | 核心职责 |
| :--- | :---: | :--- | :--- | :--- |
| **调度者** | 1 | 量化研究员（Lead） | [`./agents/quant-researcher.md`](./agents/quant-researcher.md) | 业务 Lead + 5 层 15 角色派工 |
| **分析师层** | 7 | 市场 / 新闻 / 社交媒体 / 基本面 / A股市场 / 新闻舆情 / **全球宏观** | [`./agents/quant/quant-market-analyst.md`](./agents/quant/quant-market-analyst.md) 等 7 个 | 并行收集 7 类原始信号（market / news / sentiment / fundamentals / china-market / news-social / global_macro） |
...
以下展示本体系中 9 大核心 Agent（含量化团队展开为 15 子角色）与其他专项 Skill 之间的调用与编排关系：
...
| **[📈 量化研究员](./agents/quant-researcher.md)** | 量化业务侧 Lead + 5 层 15 角色调度者；专精 Pandas 向量化、回测偏误防御、交易摩擦建模 | `factor-engineering`, `backtest-validation`, `factor-mining` + 6 个 [`./skills/trading-strategies/`](./skills/trading-strategies/) 策略文件 |
...
| [trading-strategies/](./skills/trading-strategies/) | 6 个策略知识包（基本面/技术/舆情/裁决/风控交易/全球宏观），15 角色量化团队必读 | 量化团队 15 角色 |
